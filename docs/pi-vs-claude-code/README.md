...
## In pi-multi-team-local

This vendored doc tree is the **Pi CLI playground** mirror. From the repo root:

- Run stacks with **`npm run pi-play:*`** (see root [README.md](../../README.md) → *Transcript feature coverage*).
- Smoke-test extension load without a full TUI: **`npm run pi-play:verify`**.
