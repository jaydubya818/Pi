...
> **pi-multi-team-local:** paths below are relative to the repo root (where `config/multi-team.yaml` lives). For your own fork, change `LIBRARY_REPO_URL` and use `git`/`gh` against that remote.

- **LIBRARY_REPO_URL**: `https://github.com/disler/the-library.git`
- **LIBRARY_YAML_PATH**: `vendor/library/library.yaml`
- **LIBRARY_SKILL_DIR**: `vendor/library/`
