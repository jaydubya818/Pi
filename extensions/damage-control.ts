...
export default function (pi: ExtensionAPI) {
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		ctx.ui.setStatus(
			"damage-control",
			`🛡️ Active: ${rules.bashToolPatterns.length + rules.zeroAccessPaths.length + rules.readOnlyPaths.length + rules.noDeletePaths.length} rules`,
		);
	});
	...
	pi.on("tool_call", async (event, ctx) => {
		...
		if (violationReason) {
			if (shouldAsk) {
				...
				if (!confirmed) {
					ctx.ui.setStatus("damage-control-alert", `⚠️ Blocked: ${violationReason.slice(0, 40)}…`);
				...
				ctx.ui.setStatus("damage-control-alert", `⚠️ Violation: ${violationReason.slice(0, 40)}…`);
